...
def convert_value_to_string(value):
    ...
    """
    from yacraf_calculator.config.config import DECIMALS_WHEN_ROUNDING
...
def get_font(length_unit, *, canvas_and_label=None, has_line_break=False):
    ...
    """
    from yacraf_calculator.config.config import LENGTH_UNIT, FONT, FONT_DECREASE_LINE_BREAK
...
def get_text_that_fits(canvas, label, text, text_width, is_bold, length_unit):
    ...
    """
    from yacraf_calculator.config.config import OUTLINE_WIDTH
...
def delete_all(to_delete_list, manual_delete=False):
    ...
    """
    from yacraf_calculator.blocks_gui.configuration.configuration_attribute_gui import GUIConfigurationAttribute
    from yacraf_calculator.blocks_gui.connection.connection_blocks_gui import GUIConnectionCorner, GUIConnectionScalarsIndicator
